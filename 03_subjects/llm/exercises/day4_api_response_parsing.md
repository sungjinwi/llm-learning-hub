...
- token: API 입력과 출력 비용을 이해하는 기초
- context window: 긴 문서나 긴 대화를 한 번에 다 넣을 수 없는 이유
- next-token prediction: 모델이 정답을 조회하는 것이 아니라 입력을 바탕으로 생성한다는 감각

관련 보충 실습:

- `day4_llm_theory_lite_api.md`
...
- token, context window, next-token prediction이 API 사용 판단에 왜 필요한지 설명할 수 있다.
